use siddhi_rust::core::stream::stream_junction::{StreamJunction, OnErrorAction};
use siddhi_rust::core::config::{siddhi_app_context::{SiddhiAppContext, MetricsLevelPlaceholder}, siddhi_context::SiddhiContext};
...
impl Processor for RecordingProcessor {
    fn process(&self, mut chunk: Option<Box<dyn siddhi_rust::core::event::complex_event::ComplexEvent>>) {
        while let Some(mut ce) = chunk {
            chunk = ce.set_next(None);
            if let Some(se) = ce.as_any().downcast_ref::<siddhi_rust::core::event::stream::StreamEvent>() {
...
fn test_fault_stream_routing() {
    ...
    let mut app_ctx = SiddhiAppContext::new(Arc::clone(&siddhi_context), "App".to_string(), Arc::clone(&app), String::new());
    app_ctx.set_root_metrics_level(MetricsLevelPlaceholder::BASIC);
    let def = Arc::new(StreamDefinition::new("In".to_string()).attribute("a".to_string(), AttrType::INT));
    let fault_def = Arc::new(StreamDefinition::new("Fault".to_string()).attribute("a".to_string(), AttrType::INT));
    let fault_junction = Arc::new(Mutex::new(StreamJunction::new("Fault".to_string(), Arc::clone(&fault_def), Arc::new(app_ctx.clone()), 8, false, None)));
    let mut main_junction = StreamJunction::new("In".to_string(), Arc::clone(&def), Arc::new(app_ctx.clone()), 8, true, Some(Arc::clone(&fault_junction)));
    main_junction.set_on_error_action(OnErrorAction::STREAM);
    let rec = Arc::new(Mutex::new(Vec::new()));
    fault_junction.lock().unwrap().subscribe(Arc::new(Mutex::new(RecordingProcessor{ events: Arc::clone(&rec)})));
...
fn test_error_store_routing() {
    ...
    let app = Arc::new(siddhi_rust::query_api::siddhi_app::SiddhiApp::new("App".to_string()));
    let app_ctx = SiddhiAppContext::new(Arc::clone(&siddhi_context), "App".to_string(), Arc::clone(&app), String::new());
    let def = Arc::new(StreamDefinition::new("In".to_string()).attribute("a".to_string(), AttrType::INT));
    let mut junction = StreamJunction::new("In".to_string(), Arc::clone(&def), Arc::new(app_ctx), 8, true, None);
